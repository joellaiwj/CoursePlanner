  ...
  try {
    if (!courseCode) {
      ...
      const planners = rows.flatMap((row) => {
        try {
          const draft = JSON.parse(row.draftJson) as { courseCode?: unknown; courseTitle?: unknown };
    ...
    return Response.json({ draft: JSON.parse(row.draftJson), updatedAt: row.updatedAt });
  } catch (error) {
