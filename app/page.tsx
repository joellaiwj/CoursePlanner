...
export default function Home() {
  ...
  const shareCourse = async (course: (typeof coursePlanners)[number]) => {
    setOpenCourseMenu(null);
    const recipientEmail = window.prompt(`Share ${course.code} · ${course.title}\n\nEnter the recipient's Course Agentic Planner email address:`)?.trim().toLowerCase();
    if (!recipientEmail) return;
    if (!window.confirm(`Share a copy of ${course.code} · ${course.title} with ${recipientEmail}?\n\nTheir copy will be independent, so later edits will not affect your course.`)) return;
    setCourseAction(course.plannerKey);
    try {
      const response = await fetch("/api/planner-draft/share", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ courseCode: course.plannerKey, recipientEmail }) });
      const payload = await response.json().catch(() => ({ error: "The course could not be shared." })) as { error?: string; recipient?: { displayName?: string } };
      if (!response.ok) throw new Error(payload.error || "The course could not be shared.");
      window.alert(`A copy of ${course.code} · ${course.title} has been shared with ${payload.recipient?.displayName || recipientEmail}.`);
    } catch (error) { window.alert(error instanceof Error ? error.message : "The course could not be shared. Please try again."); }
    finally { setCourseAction(null); }
  };
  ...
  return <main className="app-shell">
    ...
    <div className="workspace">
      ...
      <section className="canvas">
        ...
        <div className="canvas-sections">{sections.map((section, index) => { const isOpen=open.includes(index), isLocked=locked.includes(index), isFocused=focusedSections.includes(index); return <article data-section-index={index} className={`${isOpen ? "canvas-card expanded" : "canvas-card"}${isFocused ? " workflow-focused" : ""}`} key={section[0]}><div className="card-bar"><button className="card-title" onClick={() => setOpen(isOpen ? open.filter(i=>i!==index) : [...open,index])}><span>{String(index+1).padStart(2,"0")}</span><b>{section[0]}</b></button><div><button className={isLocked ? "lock locked" : "lock"} onClick={() => setLocked(isLocked ? locked.filter(i=>i!==index) : [...locked,index])}>{isLocked ? "● Locked" : "○ Editable"}</button><button onClick={() => setOpen(isOpen ? open.filter(i=>i!==index) : [...open,index])}>{isOpen ? "−" : "+"}</button></div></div>{isOpen && <div className={isLocked ? "card-content locked-content" : "card-content"}><p className="section-guidance"><span>Writing guidance</span>{CANVAS_WRITING_GUIDANCE[index]}</p>{renderSectionEditor(index, isLocked)}{isLocked && <small className="edit-status">Unlock this section to edit</small>}</div>}</article>})}</div>
      </section>
